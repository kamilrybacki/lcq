...
|---|---|---|---|
| **F1** | **High** | A member whose journal is lost seals its next frames from sequence 0 with the same group key and author index: nonce reuse, keystream reuse, Poly1305 key exposure. `wire/crypto.rs` `nonce_for`; `infrastructure/log.rs` `open` counted from zero. | **Fixed (D22)**: `LogJournal::open_with_entropy` — a fresh log starts at a random sequence in [2^24, 2^31), persisted first; the node uses it. Residual: ~2^-31 per frame per pair of lives. **Also done (D27)**: the group key now follows the mission epoch, so a fleet that rotates after a loss gets a new key and the old sequence range cannot be reused under it. The derivation in this build is a labelled fixture; the property is real, the secrecy is not. **And a live instance of this was found by review (D29)**: compaction fired from inside the journal's `append` and rebuilt the file from a state the caller had not updated yet, so the record that triggered it was erased. A sequence handed out and sealed under was handed out again after a restart -- nonce reuse with no attacker and no lost journal. Fixed, and the automatic compaction path is now under test; the two tests that existed called `compact()` by hand, which is a path no running node takes. |
| **F2** | Medium | Replay windows lived only in memory: after a restart every recorded frame verified again once (cost) and the state machine alone stood between a replayed vote and a double count (it does: one vote per member per case). | **Closed (D28, corrected by D29)**. The journal records the highest sequence admitted from each sender, written only when it moves, and a restart resumes each window from it. D28's first version seeded with `mark`, which sets one bit and left the sixty-three sequences below the highest reading as new -- and a member's stage frames are consecutive, so that was the common case, not a corner. `ReplayWindow::resumed` now counts everything at or below the recorded highest as seen. Residual, liveness not safety: a frame still in flight when the process died is refused when it lands. A new epoch clears the record, because an index under a new manifest is not the same member. |
...
| **F10** | Low | Verification amplification: a member (A1) can put verifiable-looking frames on the air at the channel's rate and cost every receiver an `ed25519` check each. | **Bounded and accepted (D29)**, and the mitigation this table used to recommend has been built and removed. Airtime bounds it: at SF10 one member forces at most one check per ~1.2 s, about 1 ms on a Raspberry Pi, under 0.1 % of a core. A per-sender airtime allowance was added (D28) and taken out again, because it is charged on the claimed sender index read from the header **before** any decryption -- which is the point, since decryption is what it rations. So anyone, holding no key at all, could spend about 36 s of airtime carrying a victim's index and have that member dropped before decryption at every receiver for an hour: selective, persistent, repeatable, and cheaper than jamming, which cannot be aimed. Trading a millisecond of CPU for a targeted silencing primitive is a bad trade. What was kept is the half with no downside: a frame claiming an index **outside** the fleet is dropped before decryption, since such a claim can never verify and no honest member has such an index. Any future per-sender rate limit must be charged on an **authenticated** sender, which means after verification, and is therefore evidence for F5 rather than a defence for this row. |
