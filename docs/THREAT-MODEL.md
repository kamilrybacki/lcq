...
| **F2** | Medium | Replay windows lived only in memory: after a restart every recorded frame verified again once (cost) and the state machine alone stood between a replayed vote and a double count (it does: one vote per member per case). | **Closed (D28)**. The journal records the highest sequence admitted from each sender, written only when it moves, and a restart seeds its windows from that. The earlier fix rebuilt the windows from whatever happened to be journaled, which forgot every frame that was seen and neither witnessed nor queued -- a trigger, a request, a frame refused after verifying -- so each of those could be replayed once per restart. Residual, and it is liveness not safety: seeding from the highest alone drops the in-window bitmap, so a frame that arrives out of order across a restart is refused. Stricter than what it replaces, never looser. A new epoch clears the record, because an index under a new manifest is not the same member. |
...
| **F10** | Low | Verification amplification: a member (A1) can put verifiable-looking frames on the air at the channel's rate and cost every receiver an `ed25519` check each. | **Done (D28)**, as far as an unauthenticated sender allows. Each sender has an allowance, and it is the duty cycle -- the one limit every member on this band is already bound by, so a receiver accepts exactly what an honest sender was permitted to transmit and refuses the first frame that sender's own budget would have refused. Over it, the frame is dropped **before** decryption and the signature check, metered as `over_allowance_dropped` and logged with the claimed index. The budget is not charged when it refuses, so this is a rate limit and not a ban. **The index it keys on is a claim**: it comes from the frame header, which only `open_frame` authenticates, and the check has to run before the work it is rationing. A liar can therefore spend another member's allowance. What it still buys is a ceiling on the total work one radio can force. Per-sender fairness needs an authenticated sender, which costs the check being rationed. |
