...
| **F4** | **High** (deployment) | Keys are constants: `GROUP_KEY`, `seed_for(index)`, `MISSION_EPOCH`, `EVENT`, `REVISION` in `lcq-node`. Fine for a harness, fatal for a vessel. No provisioning, rotation or manifest distribution exists. | **Open — product**. Design needed: per-member signing key on the device (never in the image), group key and manifest per mission from Vault/sops through Morsik, epoch rotation on every mission, revocation = new manifest + new group key. |
