...
|---|---|---|---|
| **F1** | **High** | A member whose journal is lost seals its next frames from sequence 0 with the same group key and author index: nonce reuse, keystream reuse, Poly1305 key exposure. `wire/crypto.rs` `nonce_for`; `infrastructure/log.rs` `open` counted from zero. | **Fixed (D22)**: `LogJournal::open_with_entropy` — a fresh log starts at a random sequence in [2^24, 2^31), persisted first; the node uses it. Residual: ~2^-31 per frame per pair of lives. Recommended in addition: bind the group key to the mission epoch so a new life gets a new key (F4). |
...
| **F4** | **High** (deployment) | Keys are constants: `GROUP_KEY`, `seed_for(index)`, `EVENT`, `REVISION` in `lcq-node`. Fine for a harness, fatal for a vessel. Since D25 the fleet's membership, competences and mission epoch come from an unsigned manifest file -- the shape the lifecycle will take, trusted as far as the file system, carrying no keys yet. No provisioning or rotation exists. | **Open — product**. Design needed: per-member signing key on the device (never in the image), group key and manifest per mission from Vault/sops through Morsik, epoch rotation on every mission, revocation = new manifest + new group key. |
...
1. **Provisioning design (F4, F21)** — the trust root is settled: the fleet's
   administrator issues the administration key and a person types it in (D26),
   and `wire::hand` is the encoding that survives being read aloud. What is
   left, before any vessel: manifest version 2,
